...
public class Allocate_minimum_number_of_pages {
    ...
    static boolean isDistributionPossible(int[] A, int N, int M, long capacity) {
		int studs = 1;
		long distributed = 0l;
		for(int i=0; i<N; i++) {
			distributed += (long) A[i];
			if(distributed > capacity) {
				studs++;
				distributed = (long) A[i];
			}
		}
		if(studs > M) return false;
		return true;
	}

	public static int findPages(int[] A, int N, int M) {
	    if(M > N) return -1;
		long low = (long) A[N-1], high = 0l, mid;
		for(int i=0; i<N; i++) high += (long) A[i];
		while(low <= high) {
			mid = low + (high-low)/2;
			if(isDistributionPossible(A, N, M, mid) == true) high = mid-1;
			else low = mid+1;
		}
		return (int) low;
	}
